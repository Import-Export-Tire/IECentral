...
function PersonnelContent() {
  ...
  // Browse mode: paginated list ordered by lastName
  const browse = usePaginatedList(
    api.personnel.list,
    searching
      ? "skip"
      : {
  ...
  // Search mode: full-text search via search index
  const search = usePaginatedList(
    api.personnel.searchPersonnel,
    searching
      ? {
  ...
  // Terminated section: separate paginated browse (always active when section is expanded)
  const terminatedBrowse = usePaginatedList(
    api.personnel.list,
    showTerminated ? { status: "terminated" } : "skip",
    { initialNumItems: 50 }
  );

  const active = searching ? search : browse;

  // Client-side showTempsOnly filter applied on the loaded page results
  const visibleResults = showTempsOnly
    ? active.results.filter((p) => isTemp(p.employeeType))
    : active.results;
  ...
  const renderPersonRow = (person: typeof active.results[0]) => (
    <tr
      key={person._id}
  ...
  const renderTerminatedRow = (person: typeof terminatedBrowse.results[0]) => (
    <tr
      key={person._id}
            ...
            <div className="overflow-x-auto">
              {active.status === "LoadingFirstPage" ? (
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {visibleResults.map((person) => renderPersonRow(person))}
                  </tbody>
                </table>
              )}
          ...
          <div className={`rounded-xl overflow-hidden border ${isDark ? "bg-slate-800/20 border-slate-700" : "bg-gray-50/80 border-gray-200"}`}>
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-sm font-medium theme-text-secondary">
                  ...
                  {showTerminated && terminatedBrowse.status !== "LoadingFirstPage" && terminatedBrowse.results.length > 0
                    ? ` (Showing ${terminatedBrowse.results.length}${terminatedBrowse.status !== "Exhausted" ? "+" : ""})`
                    : ""}
                </span>
            ...
            {showTerminated && (
              <div className={`border-t ${isDark ? "border-slate-700" : "border-gray-200"}`}>
                {terminatedBrowse.status === "LoadingFirstPage" ? (
                  <div className="p-4">
                    <Skeleton className="h-12 w-full mb-2" />
                    <Skeleton className="h-12 w-full mb-2" />
                    <Skeleton className="h-12 w-full mb-2" />
                  </div>
                ) : terminatedBrowse.results.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-sm theme-text-tertiary">No terminated employees found.</p>
                  </div>
                ) : (
                  <>
                    <table className="w-full">
                      ...
                      <tbody>
                        {terminatedBrowse.results.map((person) => renderTerminatedRow(person))}
                      </tbody>
                    </table>
                    {terminatedBrowse.status === "CanLoadMore" && (
                      <div className="flex justify-center py-3 border-t border-gray-200 dark:border-slate-700">
                        <button
                          onClick={() => terminatedBrowse.loadMore(50)}
                          className="px-4 py-2 text-sm font-medium rounded-lg bg-[var(--accent-primary)] text-white hover:opacity-90"
                        >
                          Load more
                        </button>
                    ...
                    {terminatedBrowse.status === "LoadingMore" && (
