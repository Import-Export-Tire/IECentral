        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <header className={`sticky top-0 z-10 border-b px-4 sm:px-6 py-3 sm:py-4 backdrop-blur-sm ${isDark ? "bg-slate-900/80 border-slate-700" : "bg-white/80 border-gray-200"}`}>
            <div className="flex items-center gap-3">
              ...
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isDark ? "bg-gradient-to-br from-amber-500/20 to-orange-600/20" : "bg-gradient-to-br from-amber-100 to-orange-100"}`}>
              ...
              <div className="min-w-0">
                <h1 className="text-xl font-bold theme-text-primary">Dealer Rebate Tool</h1>
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        ...
        label="Reports"
      />
      <p className="text-sm theme-text-secondary -mt-3 mb-1">
      ...
      {reports === null && !error && (
        <p className="text-sm theme-text-tertiary">Loading…</p>
      )}
      {reports !== null && quarters.length === 0 && !error && (
        <p className="text-sm theme-text-tertiary">No reports available yet.</p>
      )}
        ...
        <Card key={quarter}>
          <h3 className="text-sm font-semibold theme-text-primary mb-3">
            ...
            {byQuarter.get(quarter)![0]?.range && (
              <span className="ml-2 font-normal text-xs theme-text-tertiary">({byQuarter.get(quarter)![0].range})</span>
            )}
          </h3>
          <div className="grid gap-2 sm:grid-cols-2">
            {byQuarter.get(quarter)!.sort((a, b) => a.program.localeCompare(b.program)).map((r) => (
              <a
                ...
                download={r.fileName}
                className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border transition-colors theme-border-secondary ${isDark ? "hover:bg-slate-700/50" : "hover:bg-gray-50"}`}
              >
                <div className="min-w-0">
                  <div className="text-sm font-medium theme-text-primary">{r.display}</div>
                  <div className="text-xs font-mono truncate theme-text-tertiary">{r.fileName} · {r.months} mo · {r.days} days</div>
                </div>
...
function UploadTab({ isDark, userId }: { isDark: boolean; userId?: Id<"users"> }) {
  ...
  if (!dealers) {
    return <div className="text-center py-12 theme-text-tertiary">Loading dealers...</div>;
  }
  ...
  return (
    <div>
        ...
        {UPLOAD_STEPS.map((label, i) => (
          <div key={i} className="flex items-center gap-2 flex-1">
            ...
            <span className={`text-xs font-semibold uppercase tracking-wide ${
              step === i ? "theme-text-primary" : "theme-text-tertiary"
            }`}>
              {label}
            </span>
            {i < UPLOAD_STEPS.length - 1 && (
              <div className="flex-1 h-px theme-border-secondary bg-current opacity-30" />
            )}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <Card>
          <div className="ui-section-label mb-1">Upload OEA07V Report</div>
          <div
            className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-all mt-3 ${
              dragOver
          ...
          >
            ...
            <p className="font-semibold mb-1 theme-text-primary">
      ...
      {step === 1 && (
        <div className="space-y-4">
          <Card>
            <div className="ui-section-label mb-3">Select Programs to Generate</div>
            <Card tone="green" padding="sm" className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                >
                  ...
                  <p className="text-xs ml-8 theme-text-tertiary">
      ...
      {step === 2 && results && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <Card padding="sm">
              <div className="ui-section-label">Total Rows</div>
              <div className="text-3xl font-mono font-bold mt-1 theme-text-primary">{results.totalInputRows}</div>
            </Card>
            <Card padding="sm">
              <div className="ui-section-label">T-Type Rows</div>
              <div className={`text-3xl font-mono font-bold mt-1 ${isDark ? "text-orange-400" : "text-orange-600"}`}>{results.filteredRows}</div>
            </Card>
            {programs.falken && (
              <Card padding="sm">
                <div className="ui-section-label">Falken Rows</div>
                <div className="text-3xl font-mono font-bold mt-1 text-green-400">{results.falkenOut.length}</div>
                <div className="text-xs theme-text-tertiary mt-0.5">{results.falkenDealersMatched} dealers</div>
              </Card>
            )}
            {programs.milestar && (
              <Card padding="sm">
                <div className="ui-section-label">Milestar Rows</div>
                <div className="text-3xl font-mono font-bold mt-1 text-blue-400">{results.milestarOut.length}</div>
                <div className="text-xs theme-text-tertiary mt-0.5">{results.milestarDealersMatched} dealers</div>
              </Card>
            )}
          </div>
            ...
            <div className="theme-card overflow-hidden p-0">
              <div className={`px-5 py-3 flex items-center justify-between border-b-2 ${isDark ? "bg-amber-500/10 border-amber-500/40" : "bg-amber-50 border-amber-300"}`}>
                ...
                {results.falkenOut.length > 0 ? (
                  <Button variant="primary" size="sm" onClick={exportFalken}>
                    Download Falken CSV
                  </Button>
                ) : (
                  <span className="text-xs theme-text-tertiary">No matching dealers</span>
                )}
              </div>
              <div className="p-4">
                {results.falkenOut.length > 0 ? (
                  <>
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="theme-text-tertiary">
                        ...
                        <tbody>
                          {results.falkenOut.slice(0, 8).map((r, i) => (
                            <tr key={i} className="border-t theme-border-secondary">
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.FANATIC_Dealer_Account_Number}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.Invoice_Number}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.SKU}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.Date}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.Quantity}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.Price_Per_Tire}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    {results.falkenOut.length > 8 && (
                      <p className="text-xs mt-2 font-mono theme-text-tertiary">+ {results.falkenOut.length - 8} more rows in download</p>
                    )}
                  </>
                ) : (
                  <Card tone="amber" padding="sm">
                    <p className="text-sm">No Falken Fanatic dealers matched in this OEA07V.</p>
                  </Card>
                )}
              </div>
            </div>
            ...
            <div className="theme-card overflow-hidden p-0">
              <div className={`px-5 py-3 flex items-center justify-between border-b-2 ${isDark ? "bg-blue-500/10 border-blue-500/40" : "bg-blue-50 border-blue-300"}`}>
                ...
                {results.milestarOut.length > 0 ? (
                  <Button variant="primary" size="sm" onClick={exportMilestar}>
                ...
                {results.milestarOut.length > 0 ? (
                  <>
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {results.milestarOut.slice(0, 8).map((r, i) => (
                            <tr key={i} className="border-t theme-border-secondary">
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.DealerNumber}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.InvoiceNumber}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.ProductCode}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.InvoiceDate}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.Quantity}</td>
                              <td className="py-1.5 px-2 font-mono theme-text-secondary">{r.SellPricePerTire}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    {results.milestarOut.length > 8 && (
                      <p className="text-xs mt-2 font-mono theme-text-tertiary">+ {results.milestarOut.length - 8} more rows in download</p>
                    )}
                  </>
                ) : (
                  <Card tone="amber" padding="sm">
                    <p className="text-sm">No Milestar Momentum dealers matched in this OEA07V.</p>
                  </Card>
                )}
              </div>
            </div>
      ...
      <Card padding="sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input
            ...
            className="theme-input flex-1 min-w-[250px] px-3 py-2 text-sm"
          />
          <select
            value={programFilter}
        ...
        {/* Stats */}
        {dealers && (
          <div className="flex gap-4 mt-3 pt-3 border-t theme-border-secondary">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b theme-border-secondary ${isDark ? "bg-slate-800/80" : "bg-gray-50"}`}>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">Name</th>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">JMK</th>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">Fanatic ID</th>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">Momentum #</th>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">Programs</th>
                <th className="text-left py-3 px-4 font-semibold text-xs theme-text-tertiary">Status</th>
                <th className="text-right py-3 px-4 font-semibold text-xs theme-text-tertiary">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredDealers.map(d => (
                <tr key={d._id} className={`border-t theme-border-secondary transition-colors ${isDark ? "hover:bg-slate-800/50" : "hover:bg-gray-50"} ${!d.isActive ? "opacity-50" : ""}`}>
                  <td className="py-2.5 px-4 font-medium theme-text-primary">{d.name}</td>
                  <td className="py-2.5 px-4 font-mono text-xs theme-text-secondary">{d.jmk || "—"}</td>
                  <td className="py-2.5 px-4 font-mono text-xs theme-text-secondary">{d.fanaticId ?? "—"}</td>
                  <td className="py-2.5 px-4 font-mono text-xs theme-text-secondary">{d.dealerNumber ?? "—"}</td>
                  <td className="py-2.5 px-4">
                    <div className="flex gap-1">
                      {d.programs.includes("falken") && (
                        <span className="ui-badge ui-badge-amber">Falken</span>
                      )}
                      {d.programs.includes("milestar") && (
                        <span className="ui-badge ui-badge-blue">Milestar</span>
                      )}
                    </div>
                  </td>
                  <td className="py-2.5 px-4">
                    {d.isActive ? (
                      <span className="ui-badge ui-badge-green">Active</span>
                    ) : (
                  ...
                  <td className="py-2.5 px-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {canDeactivate && (
                        d.isActive ? (
                          <Button variant="danger" size="sm" onClick={() => setConfirmDeactivate(d)}>
                            Deactivate
                          </Button>
                        ) : (
                          <button
              ...
              {filteredDealers.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-sm theme-text-tertiary">
                    No dealers found
                  </td>
                </tr>
              )}
          ...
          <div className="w-full max-w-md p-4" onClick={e => e.stopPropagation()}>
            <Card padding="md">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium mb-1 theme-text-tertiary">Dealer Name *</label>
                  <input
                    type="text"
                    value={formName}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium mb-1 theme-text-tertiary">JMK Account #</label>
                    <input
                      type="text"
                      value={formJmk}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 theme-text-tertiary">Prim/Sec</label>
                    <select
                      value={formPrimSec}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium mb-1 theme-text-tertiary">Fanatic ID</label>
                    <input
                      type="number"
                      value={formFanaticId}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 theme-text-tertiary">Momentum #</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-xs font-medium mb-1 theme-text-tertiary">Programs</label>
                  <div className="flex gap-3 mt-1">
                    {["falken", "milestar"].map(p => (
                      <label key={p} className="flex items-center gap-2 text-sm theme-text-primary">
                        <input
                          type="checkbox"
              ...
              <div className="flex justify-end gap-3 mt-6">
                <Button variant="ghost" onClick={() => { setShowAddModal(false); setEditingDealer(null); }}>
                  Cancel
                </Button>
                <Button
                  variant="primary"
          ...
          <div className="w-full max-w-sm p-4" onClick={e => e.stopPropagation()}>
            <Card padding="md">
              ...
              <p className="text-sm mb-4 theme-text-secondary">
                Are you sure you want to deactivate <strong className="theme-text-primary">{confirmDeactivate.name}</strong> (JMK: {confirmDeactivate.jmk || "N/A"})? They will no longer appear in upload processing.
              </p>
...
function UploadHistoryTab({ isDark }: { isDark: boolean }) {
  ...
  const renderSourceCard = (u: any) => (
    <div
      key={u._id}
      className={`theme-card p-4 transition-all cursor-pointer ${
        selectedUploadId === u._id
          ? isDark ? "ring-2 ring-orange-500/40" : "ring-2 ring-orange-300"
          : ""
      }`}
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className={`ui-badge ${u.program === "falken" ? "ui-badge-amber" : "ui-badge-blue"}`}>{u.program}</span>
          <span className="text-sm font-medium theme-text-primary">{u.fileName}</span>
        </div>
        <span className="text-xs theme-text-tertiary">{formatDate(u.uploadDate || u.createdAt)}</span>
      </div>
      <div className="flex gap-4 mt-2 text-xs theme-text-tertiary">
      ...
      {selectedUploadId === u._id && (
        <div className="mt-4 pt-4 border-t theme-border-secondary">
          {(("dealerBreakdown" in u && u.dealerBreakdown) || ("matchedDealers" in u && u.matchedDealers)) && (
            <div className="mb-3">
              <div className="text-xs font-medium mb-2 theme-text-secondary">{"matchedDealers" in u && u.matchedDealers ? "Matching Dealers:" : "Dealer Breakdown:"}</div>
              <div className="grid grid-cols-2 gap-1">
                {((u.matchedDealers ?? u.dealerBreakdown) as any[]).map((d: any, i: number) => (
                  <div key={i} className={`flex justify-between text-xs py-1 px-2 rounded ${isDark ? "bg-slate-700/50" : "bg-gray-50"}`}>
                    <span className="theme-text-primary">{d.name}</span>
                    <span className="font-mono theme-text-tertiary">{d.qty ?? d.rowCount} {d.qty != null ? "tires" : "rows"}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            {selectedUpload?.resultData && (
              <Button variant="primary" size="sm" onClick={e => { e.stopPropagation(); reExport(); }}>
                Re-export CSV
              </Button>
            )}
            {selectedUpload && !selectedUpload.resultData && (
              <span className="text-xs theme-text-tertiary">Loading export data...</span>
            )}
            {canDeleteUploads && (
              <Button
                variant="danger"
  ...
  return (
    <div className="space-y-4">
      {/* Controls */}
      <Card padding="sm">
        <SectionHeader label="Upload History" />
        <div className="flex flex-wrap items-center gap-3">
          <input
            type="text"
      ...
      {debouncedSearch.length >= 2 && searchMonthly && searchMonthly.length > 0 && (
        <div className="theme-card overflow-hidden p-0">
          <div className={`px-5 py-3 border-b theme-border-secondary ${isDark ? "bg-slate-800/80" : "bg-gray-50"}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b theme-border-secondary ${isDark ? "bg-slate-800/60" : "bg-gray-50"}`}>
                  <th className="text-left py-2 px-4 font-semibold text-xs theme-text-tertiary">Month</th>
                  <th className="text-right py-2 px-4 font-semibold text-xs theme-text-tertiary">Falken</th>
                  <th className="text-right py-2 px-4 font-semibold text-xs theme-text-tertiary">Milestar</th>
                  <th className="text-right py-2 px-4 font-semibold text-xs theme-text-tertiary">Total</th>
                </tr>
              </thead>
              <tbody>
                {searchMonthly.map((m) => (
                  <tr key={m.month} className="border-t theme-border-secondary">
                    <td className="py-2 px-4 font-medium theme-text-primary">{m.month}</td>
                    ...
                    <td className="py-2 px-4 text-right font-mono font-bold theme-text-primary">{m.total || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {debouncedSearch.length >= 2 ? (
        <div className="space-y-3">
          {searchUploads && searchUploads.length > 0
            ...
            : <div className="text-center py-12 theme-text-tertiary">No records matching your search</div>}
        </div>
      ) : (
        <>
          {/* Daily Submission Files — the deliverables to upload to the portal */}
          <div className="flex items-end justify-between">
            <span className="ui-section-label">Daily Submission Files</span>
            <span className="text-xs theme-text-tertiary">one file per brand per day · download to submit</span>
          </div>
          {dailyByMonth.length === 0 ? (
            <div className="text-center py-12 theme-text-tertiary">No submission files yet</div>
          ) : (
            <div className="space-y-3">
              {dailyByMonth.map(({ month, days }) => {
                const open = !collapsedMonths.has(month);
                return (
                  <div key={month} className="theme-card overflow-hidden p-0">
                    <button
                    ...
                    >
                      <span className="text-sm font-bold theme-text-primary">{monthLabel(month)}</span>
                      <span className="text-xs theme-text-tertiary">{open ? "▾" : "▸"} {days.length} day{days.length !== 1 ? "s" : ""}</span>
                    </button>
                    {open && (
                      <div className="divide-y theme-border-secondary">
                        {days.map(({ day, falken, milestar }) => (
                          <div key={day} className="flex items-center justify-between px-5 py-2.5">
                            <span className="text-sm font-medium theme-text-primary">{dayLabel(day)}</span>
                            <div className="flex items-center gap-2">
                              {falken
                                ...
                                : <span className="px-3 py-1 text-xs theme-text-tertiary">Falken —</span>}
                              {milestar
                                ...
                                : <span className="px-3 py-1 text-xs theme-text-tertiary">Milestar —</span>}
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            {showIngested && (
              <div className="space-y-3 mt-3">
                {convexUploads.length === 0
                  ? <div className="text-center py-6 text-xs theme-text-tertiary">No ingested files</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setConfirmDeleteId(null)}>
          <div className="w-full max-w-sm p-4" onClick={e => e.stopPropagation()}>
            <Card padding="md">
              ...
              <p className="text-sm mb-4 theme-text-secondary">
                Are you sure you want to permanently delete the upload record for <strong className="theme-text-primary">{confirmDeleteName}</strong>? This cannot be undone.
              </p>
              <div className="flex justify-end gap-3">
...
function StatsTab({ isDark }: { isDark: boolean }) {
  ...
  if (!uploads || !dealers || !statsData) {
    return <div className="text-center py-12 theme-text-tertiary">Loading stats...</div>;
  }

  if (!stats) {
    return <div className="text-center py-12 theme-text-tertiary">No upload data yet. Process some OEA07V files to see stats.</div>;
  }

  const growthColor = (val: number | null) => {
    if (val === null) return "theme-text-tertiary";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card padding="sm">
          ...
          <div className="text-3xl font-mono font-bold mt-1 theme-text-primary">{stats.currentMonth.total.toLocaleString()}</div>
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs font-bold ${growthColor(monthGrowth)}`}>{growthArrow(monthGrowth)}</span>
            <span className="text-xs theme-text-tertiary">vs last month</span>
          </div>
        </Card>

        <Card padding="sm">
          <div className="ui-section-label">Year to Date</div>
          <div className="text-3xl font-mono font-bold mt-1 theme-text-primary">{stats.currentYear.total.toLocaleString()}</div>
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs font-bold ${growthColor(stats.yoyGrowth)}`}>{growthArrow(stats.yoyGrowth)}</span>
            <span className="text-xs theme-text-tertiary">YoY</span>
          </div>
        </Card>

        <Card padding="sm">
          <div className={`ui-section-label ${isDark ? "text-amber-400/70" : "text-amber-600"}`}>Falken YTD</div>
          <div className="text-3xl font-mono font-bold text-amber-400 mt-1">{stats.currentYear.falken.toLocaleString()}</div>
        ...
        <Card padding="sm">
          <div className={`ui-section-label ${isDark ? "text-blue-400/70" : "text-blue-600"}`}>Milestar YTD</div>
          <div className="text-3xl font-mono font-bold text-blue-400 mt-1">{stats.currentYear.milestar.toLocaleString()}</div>
          <div className="text-xs mt-1 theme-text-tertiary">
      ...
      {/* Monthly Chart */}
      <Card>
        <SectionHeader label="Monthly Volume (Last 12 Months)" />
              ...
              <div key={m.key} className="flex-1 flex flex-col items-center gap-1 group relative">
                <div className="w-full flex flex-col justify-end h-40">
                  {m.total > 0 ? (
                    ...
                    <div className="w-full h-0.5 rounded bg-current opacity-20" />
                  )}
                </div>
                <span className="text-[9px] font-mono theme-text-tertiary">{m.label}</span>
                {/* Tooltip */}
                <div className="absolute bottom-full mb-2 hidden group-hover:block z-10 px-2 py-1 rounded text-[10px] whitespace-nowrap bg-gray-800 text-white">
        ...
        <div className="flex items-center gap-4 mt-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs theme-text-tertiary">Falken</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded bg-blue-500/80" />
            <span className="text-xs theme-text-tertiary">Milestar</span>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="theme-card overflow-hidden p-0">
          <div className={`px-5 py-3 border-b theme-border-secondary ${isDark ? "bg-slate-800/80" : "bg-gray-50"}`}>
            <h3 className="text-sm font-bold theme-text-primary">Monthly Breakdown</h3>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[...stats.months].reverse().map(m => (
                  <tr key={m.key} className="border-t theme-border-secondary">
                    <td className="py-2 px-4 font-medium theme-text-primary">{m.label}</td>
                    <td className="py-2 px-4 text-right font-mono text-amber-400">{m.falken || "—"}</td>
                    <td className="py-2 px-4 text-right font-mono text-blue-400">{m.milestar || "—"}</td>
                    <td className="py-2 px-4 text-right font-mono font-bold theme-text-primary">{m.total || "—"}</td>
                  </tr>
        ...
        <div className="theme-card overflow-hidden p-0">
          <div className={`px-5 py-3 border-b theme-border-secondary ${isDark ? "bg-slate-800/80" : "bg-gray-50"}`}>
            <h3 className="text-sm font-bold theme-text-primary">Top 10 Dealers by Volume</h3>
          </div>
          <div className="overflow-y-auto max-h-80">
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b theme-border-secondary ${isDark ? "bg-slate-800/60" : "bg-gray-50"}`}>
                  <th className="text-left py-2 px-4 font-semibold text-xs theme-text-tertiary">#</th>
                  <th className="text-left py-2 px-4 font-semibold text-xs theme-text-tertiary">Dealer</th>
                  <th className="text-right py-2 px-4 font-semibold text-xs theme-text-tertiary">Tires</th>
                </tr>
              </thead>
              <tbody>
                {stats.topDealers.map((d, i) => {
                  ...
                  return (
                    <tr key={d.name} className="border-t theme-border-secondary">
                      <td className="py-2 px-4 font-mono text-xs theme-text-tertiary">{i + 1}</td>
                      <td className="py-2 px-4">
                        <div className="font-medium text-sm theme-text-primary">{d.name}</div>
                      ...
                      <td className="py-2 px-4 text-right">
                        <div className="font-mono font-bold theme-text-primary">{d.total}</div>
                ...
                {stats.topDealers.length === 0 && (
                  <tr><td colSpan={3} className="py-8 text-center text-sm theme-text-tertiary">No dealer data yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {(() => {
        ...
        return (
          <div className="theme-card overflow-hidden p-0">
            <div className={`px-5 py-3 border-b theme-border-secondary flex items-center gap-3 ${isDark ? "bg-slate-800/80" : "bg-gray-50"}`}>
              <h3 className="text-sm font-bold theme-text-primary">Per-Dealer Monthly Totals</h3>
              <input
                type="text"
                placeholder="Search dealer name or JMK…"
                value={dealerSearch}
                onChange={e => handleDealerSearch(e.target.value)}
                className="theme-input flex-1 max-w-xs px-3 py-1.5 text-sm"
              />
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className={`border-b theme-border-secondary ${isDark ? "bg-slate-800/60" : "bg-gray-50"}`}>
                    <th className="text-left py-2 px-4 font-semibold text-xs whitespace-nowrap theme-text-tertiary">Dealer</th>
                    {sixMonthKeys.map(({ key, label }) => (
                      <th key={key} className="text-right py-2 px-3 font-semibold text-xs whitespace-nowrap theme-text-tertiary">{label}</th>
                    ))}
                    <th className="text-right py-2 px-4 font-semibold text-xs theme-text-tertiary">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {sorted && sorted.length > 0 ? sorted.map(d => (
                    <tr key={d.name} className={`border-t theme-border-secondary transition-colors ${isDark ? "hover:bg-slate-800/40" : "hover:bg-gray-50"}`}>
                      <td className="py-2 px-4 font-medium whitespace-nowrap theme-text-primary">
                        <div>{d.name}</div>
                        {d.jmk && <div className="text-[10px] font-mono theme-text-tertiary">{d.jmk}</div>}
                      </td>
                      {sixMonthKeys.map(({ key }) => {
                        const val = (d.months[key]?.falken ?? 0) + (d.months[key]?.milestar ?? 0);
                        return (
                          <td key={key} className="py-2 px-3 text-right font-mono text-xs theme-text-secondary">
                            {val > 0 ? val : "—"}
                          </td>
                        );
                      })}
                      <td className="py-2 px-4 text-right font-mono font-bold text-xs theme-text-primary">
                        {d._total}
                      </td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan={sixMonthKeys.length + 2} className="py-8 text-center text-sm theme-text-tertiary">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div>
            <div className="text-xs font-medium mb-2 theme-text-secondary">Combined</div>
            <div className="flex items-end gap-3">
              <div>
                <div className="text-2xl font-mono font-bold theme-text-primary">{stats.cyToDate.toLocaleString()}</div>
                <div className="text-xs theme-text-tertiary">{new Date().getFullYear()} YTD</div>
              </div>
              <div>
                <div className="text-lg font-mono theme-text-tertiary">{stats.lyToDate.toLocaleString()}</div>
                <div className="text-xs theme-text-tertiary">{new Date().getFullYear() - 1} YTD</div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="flex items-end gap-3">
              <div>
                ...
                <div className="text-xs theme-text-tertiary">{new Date().getFullYear()}</div>
              </div>
              <div>
                <div className="text-lg font-mono theme-text-tertiary">{stats.lastYear.falken.toLocaleString()}</div>
                <div className="text-xs theme-text-tertiary">{new Date().getFullYear() - 1}</div>
              </div>
            </div>
            {stats.lastYear.falken > 0 && (
          ...
          <div>
            ...
            <div className="flex items-end gap-3">
              ...
              <div>
                <div className="text-lg font-mono theme-text-tertiary">{stats.lastYear.milestar.toLocaleString()}</div>
                <div className="text-xs theme-text-tertiary">{new Date().getFullYear() - 1}</div>
              </div>
            </div>
            {stats.lastYear.milestar > 0 && (
