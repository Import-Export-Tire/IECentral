...
- **Sync (IMAP):** `triggerSync` → `ensureValidToken` → incremental (UID cursor, inbox+sent, 2-hr date fallback) or full (30-day search, batches of 25). `lastSyncUid` only advances after a fully successful fetch — on a mid-fetch error the cursor is intentionally left in place so the next run retries the same window (`convex/email/sync.ts:790`, `:798`). The **overlap guard** at the top of `performIncrementalSync` (`sync.ts:605`) short-circuits if `syncStatus === "syncing"` AND `Date.now() - updatedAt < 3*60*1000`; a `"syncing"` status older than the 3-min window is treated as a stale lock from a crashed run and allowed through. Connection establishment goes through `connectWithRetry` (`sync.ts:206`): up to 3 attempts with **linear backoff** `2000*(i+1)` ms, retrying only transient errors matching `/ECONNREFUSED|ETIMEDOUT|timeout|ECONNRESET|EHOSTUNREACH/i` and **failing fast on auth errors**. TLS uses `rejectUnauthorized:false` for `svm.ietires.com`'s self-signed cert (`sync.ts:324`, `:663`).
...
- **WebRTC signaling = Convex as the signaling server.** Full mesh (one `RTCPeerConnection` per remote). The signaling backend (`convex/meetingSignaling.ts`) is deliberately tiny — three mutations (`sendSignal`, `consumeSignal`, `cleanupSignals`) and one query (`getMySignals`). The end-to-end message flow:
  1. **Role assignment** — deterministic initiator via string comparison `myId < remoteId` (the "impolite"/initiator peer); the other is "polite". This avoids glare without a central coordinator.
  2. **Send** — every offer/answer/ICE candidate is `JSON.stringify`'d into its `payload` and written as a `meetingSignals` row via `sendSignal` (`meetingSignaling.ts:24`), keyed `{fromParticipantId, toParticipantId, type, isConsumed:false}`. ICE candidates are emitted from `pc.onicecandidate` (`usePeerConnections.ts:162`) — so **each candidate is a discrete DB insert**.
  3. **Receive** — each client subscribes via `getMySignals` (`meetingSignaling.ts:7`), which reads the `by_recipient` index `[toParticipantId, isConsumed=false]` — the hot path; Convex pushes new rows reactively (no polling). The client processes each, then calls `consumeSignal` to patch `isConsumed:true` so it isn't re-handled.
  4. **Perfect negotiation** (`usePeerConnections.ts:241`+) — on an inbound **offer**, an offer collision (`peer.makingOffer || signalingState !== "stable"`) is *discarded* by the impolite peer (`:251`) and accepted (rollback-style) by the polite peer; otherwise `setRemoteDescription` → `createAnswer` → `setLocalDescription` → send answer. On an **answer**, applied only in `have-local-offer` state. **ICE candidates** are buffered per-peer in `pendingCandidatesRef` and only added once `pc.remoteDescription` is set (`:296`); the buffer is flushed immediately after each remote description is applied (`:260`, `:287`).
  5. **Cleanup** — `cleanupSignals` (`meetingSignaling.ts:63`) does a `by_meeting` scan and deletes only the `isConsumed` rows. **This is the only reaper** — there is no cron for `meetingSignals`, so if a meeting never calls it the table accumulates consumed rows (see gotchas).
...
- **Audio → AI notes (noted meetings):** record mixed audio → presigned **S3 PUT** via `app/api/meetings/upload-audio/route.ts` (bucket `iecentral-meeting-recordings`) → `transcribeAndGenerateNotes` (`convex/meetingNoteActions.ts`) runs a linear status machine — the `meetingNotes.status` field steps `recording`→`uploading`→`transcribing`→`generating`→`complete` (or `error`, with `errorMessage`):
  1. **Transcribe** — set status `transcribing` (`meetingNoteActions.ts:50`), download the S3 audio, then **OpenAI Whisper** `whisper-1` via a **hand-built multipart body** (`Content-Disposition` boundary assembled by hand at `:75`, POSTed at `:100`) because Convex's Node runtime lacks browser `FormData`. The endpoint returns `text` (not JSON) so the transcript is read with `whisperResponse.text()`.
  2. **Generate** — set status `generating`, then **Anthropic Claude** via **raw `fetch` to `https://api.anthropic.com/v1/messages`** (`:176`; SDK has Convex bundling issues), `anthropic-version: 2023-06-01`, `model: "claude-sonnet-4-6"` (`:184`), `max_tokens: 4096`, single user message. The prompt demands JSON-only output; the response text is matched with a `/\{[\s\S]*\}/` regex and `JSON.parse`d into structured fields (summary/actionItems[]/decisions[]/followUps[]/keyTopics[]). actionItems are normalized to `{text, assignee?, dueDate?, completed:false}`.
  3. **Notify** — fan out an in-app `notifications.create` to every participant with a `userId` (which in turn schedules web push).

  Two graceful-degradation branches: missing `OPENAI_API_KEY` substitutes a placeholder transcript and skips Whisper (`:55`); missing `ANTHROPIC_API_KEY` writes a placeholder summary and skips Claude (`:127`); a Claude error inside the try/catch (`:225`) stores "AI note generation failed. The transcript is available below." with empty arrays rather than failing the whole action. Notes viewer has 5-step progress, action-item checkboxes, Save-to-DocHub (HTML export).
  > **Model-id note:** the live Claude model is **`claude-sonnet-4-6`** (`convex/meetingNoteActions.ts:184`) — *changed since the 2026-06-22 revision of this doc, which cited `claude-sonnet-4-20250514`.* This is the only Anthropic call in the comms cluster.
...
- **Programmatic creation + token refresh:** `zoomMeetings.createZoomMeeting` (`convex/zoomMeetings.ts`) decrypts the stored access token (`:53`); if `tokenExpiresAt < Date.now() + 60000` (i.e. expired or within a 60-second window, `:59`) it decrypts the refresh token and POSTs `grant_type=refresh_token` to `https://zoom.us/oauth/token` (`:65`). On success it **re-encrypts and persists both** the new access *and* refresh token plus a recomputed `tokenExpiresAt = Date.now() + expires_in*1000` (`:84`–`:92`) — Zoom rotates the refresh token on every refresh, so failing to persist the new one would break the next refresh. A failed refresh throws "Try reconnecting Zoom." Only then does it POST `api.zoom.us/v2/users/me/meetings` (cloud auto-record). `attachZoomToEvent` patches the created meeting onto a calendar event.
...
WebRTC P2P mesh (Convex signaling, Google STUN + Metered.ca TURN), OpenAI Whisper (`whisper-1`), Anthropic Claude (`claude-sonnet-4-6`), AWS S3 (recordings), Resend (invites), Zoom OAuth + REST, MediaPipe (virtual bg), native Companion App (local WS), DocHub (notes export).

### Gotchas
- Convex is the signaling server — every ICE candidate is a discrete DB write + reactive read; `meetingSignals` rows are only pruned by `cleanupSignals` (client-invoked, `isConsumed`-only) and **there is no cron**, so a meeting that never calls it leaves consumed rows behind permanently.
...
**Significant gotcha (still present — verified):** the app uses **next-pwa** (`next.config.ts:8`, Workbox SW `public/sw.js` at scope `/`, with `register:true, skipWaiting:true` and `clientsClaim:true, cleanupOutdatedCaches:true` widened in via `as any` at `:22`). That generated `sw.js` (27 KB Workbox precache shell) has **no `push` listener**. The subscribe path in `lib/useWebPush.ts:77` does `navigator.serviceWorker.getRegistration("/")` and, **if `existing?.active`, reuses it** (`:78`); it only registers the minimal push-only `public/push-sw.js` when no SW is active (`:82`). With `register:true + skipWaiting + clientsClaim`, the Workbox SW is active first in production, so `pushManager.subscribe()` binds to the SW that *can't* show notifications — delivered pushes (DMs, in-app notification bridge) may never display. `push-sw.js` (which does handle `push`→`showNotification` and `notificationclick`→focus/navigate) effectively only runs where no next-pwa SW is active (e.g. dev). The likely real fix is to merge the push handlers into the Workbox SW (custom `importScripts`/`workboxOptions`) or register `push-sw.js` at a distinct scope. **Confirmed in source as of this revision; affects production push delivery.**
...
- **Crons (`convex/crons.ts`):** the email client owns essentially the entire comms cron suite — nine jobs:
  | Cron | Schedule | Target |
  |---|---|---|
  | `email-sync-all-accounts` | every **5 min** | `email.sync.syncAllAccounts` (the fail2ban-safe background poll) |
  | `email-process-scheduled-sends` | every **1 min** | `email.send.processScheduledSends` |
  | `email-retry-failed-sends` | every **1 min** | `email.send.retryFailedSends` |
  | `email-process-snoozed` | every **1 min** | `email.snooze.processDueSnoozes` (resurfaces due mail as unread) |
  | `email-cleanup-old-emails` | daily | `email.emails.cleanupOldEmails` (>30-day rolling cache) |
  | `email-cleanup-send-queue` | daily | `email.sendMutations.cleanupOldQueueEntries` |
  | `email-cleanup-read-receipts` | daily | `email.readReceipts.cleanup` |
  | `email-cleanup-audit-logs` | weekly | `email.audit.cleanup` |
  | `email-cleanup-analytics` | monthly | `email.analytics.cleanup` |

  Everything else in this cluster has **no cron**: no notification, push, calendar/event, `meetingSignals`, or `meetingNotes` reaper. (The only other crons in the file are unrelated — dealer-rebate cleanup and two `scannerMdm` jobs.) `pushNotifications.sendClockOutReminders` is defined but unscheduled (orphaned — verified: only reference outside `_generated` is its own definition at `convex/pushNotifications.ts:40`).
